...
export default function AdminResourceManager({ title, singular, endpoint, fields, columns, canDelete = true, transformSubmit, extraActions, filters = [], Layout = AdminLayout }) {
  ...
  const optionValue = (option) => typeof option === 'object' ? option[field.optionValue || 'name'] : option;
  const optionLabel = (option) => typeof option === 'object' ? option[field.optionLabel || 'name'] : option;
